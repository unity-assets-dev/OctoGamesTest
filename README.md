...
И впринципе позволяет осуществлять подмену места записи (EasySave3/PlayerPrefs/File/Stream etc) через условные операторы или внедрение внешних контейнеров через фабрику.
...
В дополнение к обычному подходу, для легкости переиспользования и чистоты кода, я так же реализовываю систему плагинов\презентеров совместно с расширениями
...
Но, как и у любой гибкой системы - тут главное не переборщить.
...
public class CharactersView : MonoBehaviour {
	...
	private void FixedUpdate() {
		// Ограничим обновление раз в секунду
		if((_timer += Time.fixedDeltaTime) < 1) return;
